...
class Pegawai_model extends BaseModel {
	...
	public function get_one($id_pegawai) {
		...
		$this->db
			...
			->join('m_spesialis n', 'a.id_kota=n.id', 'left')
			->join('m_profesi o', 'a.id_kota=o.id', 'left')
		...
		$row->data_keluarga = $this->db
			...
			->select([
				...
				'e.nama AS tempat_lahir',
				'f.nama AS tempat_menikah',
				'g.nama AS suku',
		...
		if(!empty($fields['data_keluarga'])) {
			...
			foreach ($fields['data_keluarga']['id_agama'] as $key => $value) {
				$sql .= $this->db->compile_binds('(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?),', [
					...
					$this->m_kota->insert_if_not_exist($fields['data_keluarga']['tempat_lahir'][$key]),
					$this->m_kota->insert_if_not_exist($fields['data_keluarga']['tempat_menikah'][$key]),
					$this->m_suku->insert_if_not_exist($fields['data_keluarga']['suku'][$key]),
